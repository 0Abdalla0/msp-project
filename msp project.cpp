...
struct transaction
{
    string inputamount1;
    string inputamount2;
    string inputdepositstatment;
    string inputwithdrawstatment;
};
...
}usernamebox, emailbox, passbox, phoneno, Age, signinbox, balancebox, signupbox, emailsigninbox, passsigninbox, ButtonSigninBox, backbox, depositbox, withdrawbox, profilebox;;
//----------------------------------------- BODY SETUP ----------------------------------------------------------
struct bodyText
{
    Text text;
}signup2, balance, signin, signintext, username, email, password, phonenum, age, inputTextusername, inputtextemail, inputtextpass, inputtextnum, inputtextage, inputtextbalance, emailTextsignin, passwordTextsignin, textbalance, transactions, deposit, withdraw, deposittext, withdrawtext, amounttext1, amounttext2, profile, updatedBalancetext;
//----------------------------------------- TEXT SETUP ------------------------------------------------------------
...
{
    int UserIndex = 0;
    object Button, Background, errorline0, errorline, errorline2, Button0, typeline, typeline2, typeline3, typeline4, typeline5, typeline6, typeline7, typeline8, SignInBackground, lockIcon, loginIcon, errorline3, errorline4, errorline5, errorline6, errorline7, back, transback, typeline9, typeline10, button1;
    ...
    //----------------------------------------Transactionspage------------------------------------------
    transback.setupObject("transback.png", 0, 0);
    typeline9.setupObject("Type Line.png", 220, 355);
    typeline10.setupObject("Type Line.png", 220, 485);
    button1.setupObject("button.png", 1070, 620);

    //----------------------------------------Hit boxes in transactions page----------------------------
    depositbox.setup(470, 30, 220, 325);
    withdrawbox.setup(470, 30, 220, 455);
    profilebox.setup(230, 60, 1200, 725);
    //-------------------------------transactions text------------------------------
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::TextEntered)
            {
                ...
                if (page3)
                {
                    ...
                    case 1:
                    {
                        backspaceDelete(event, trans.inputamount1);
                        textSetup(amounttext1, trans.inputamount1, COURIER, 25, Color(255, 228, 201), 220, 325);
                        break;
                    }
                    case 2:
                    {
                        backspaceDelete(event, trans.inputamount2);
                        textSetup(amounttext2, trans.inputamount2, COURIER, 25, Color(255, 228, 201), 220, 455);
                        break;
                    }
        ...
        if (save)
        {
            if (!users[UserIndex].userAccount.inputbalance.empty())
            {
                firstBalance = stoll(users[UserIndex].userAccount.inputbalance); // Assuming initial balance
            }
            save = false; // WHEN SIGN OUT IT WILL RETUTN TRUE
            isEnterPressed = false;
        }
        ...
        //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        //  (------------------------------------- CHECK EMAIL IS SAME ---------------------------------------------------)   (-------------------------------------- CHECK PASSWORD IS SAME ----------------------------------------------)
        if (temp.userAccount.emailSignin.empty() || temp.userAccount.emailSignin != users[UserIndex].userAccount.inputemail || temp.userAccount.passSignin.empty() || temp.userAccount.passSignin != users[UserIndex].userAccount.inputpass)
        {
            p2issafe = false;
        }
        else
        ...
        if (p3section == 1 && isEnterPressed)
        {

            textSetup(deposittext, "you have deposited:\t" + trans.inputamount1 + "$", COURIER, 35, Color(255, 228, 201), 700, 320);
            if (!trans.inputamount1.empty())
            {
                value = stoll(trans.inputamount1);
                firstBalance += value; // Add the deposited value to firstBalance
            }
            updatedBalance = to_string(firstBalance);
            textSetup(updatedBalancetext, updatedBalance + "$", COURIER, 50, Color(255, 228, 201), 850, 90);
            staydrawn = true;
        ...
        if (p3section == 2)
        {
            if (isEnterPressed)
            {
                textSetup(withdrawtext, "you have withdrawn:\t" + trans.inputamount2 + "$", COURIER, 35, Color(255, 228, 201), 700, 450);
                if (!trans.inputamount2.empty())
                {
                    value = stoll(trans.inputamount2);
                    firstBalance -= value; // Subtract the withdrawn value from firstBalance
                }
                updatedBalance = to_string(firstBalance);
                textSetup(updatedBalancetext, updatedBalance + "$", COURIER, 50, Color(255, 228, 201), 850, 90);
                staydrawn = true;
        ...
        Draw(temp, Button, Background, Button0, typeline, typeline2, typeline3, typeline4, typeline5, typeline6, typeline7, typeline8, SignInBackground, lockIcon, loginIcon, errorline0, errorline, errorline2, errorline3, errorline4, errorline5, errorline6, errorline7, intnum, intage, intbalance, back, transback, typeline9, typeline10, button1);
...
void Draw(user& temp, object& Button, object& Background, object& Button2, object& typeline, object& typeline2, object& typeline3, object& typeline4, object& typeline5, object& typeline6, object& typeline7, object& typeline8, object& SignInBackground, object& lockIcon, object& loginIcon, object& errorline0, object& errorline, object& errorline2, object& errorline3, object& errorline4, object& errorline5, object& errorline6, object& errorline7, int intnum, int intage, int intbalance, object& back, object& transback, object& typeline9, object& typeline10, object& button1)
{
    ...
    if (page3)
    {
        window.draw(transback.sprite);
        window.draw(textbalance.text);
        window.draw(transactions.text);
        window.draw(deposit.text);
        window.draw(withdraw.text);
        window.draw(typeline9.sprite);
        window.draw(typeline10.sprite);
        window.draw(amounttext1.text);
        window.draw(amounttext2.text);
        ...
        if (staydrawn)
        {
            window.draw(updatedBalancetext.text);
            window.draw(deposittext.text);
            window.draw(withdrawtext.text);
        }
...
void setBodytext(bodyText& signup2, bodyText& signin, bodyText& signintext, bodyText& username, bodyText& email, bodyText& password, bodyText& phonenum, bodyText& age, bodyText& balance)
{
    ...
    textSetup(textbalance, "Current Balance:", COURIER, 50, Color(255, 228, 201), 350, 90);
    textSetup(transactions, "Transactions", COURIER, 45, Color(255, 228, 201), 40, 200);
    textSetup(deposit, "Deposit:", COURIER, 35, Color(255, 228, 201), 50, 320);
    textSetup(withdraw, "Withdraw:", COURIER, 35, Color(255, 228, 201), 25, 450);
    textSetup(profile, "Profile", COURIER, 35, Color(33, 33, 33), 1250, 730);
...
void mousepo(Vector2i& mouseposition)
{
    ...
    if (signupbox.box.getGlobalBounds().contains(mouseposition.x, mouseposition.y) && Mouse::isButtonPressed(Mouse::Left) && safe)
    {
    ...
    if (depositbox.box.getGlobalBounds().contains(mouseposition.x, mouseposition.y) && Mouse::isButtonPressed(Mouse::Left))
    {
        p3section = 1;
    }
    if (withdrawbox.box.getGlobalBounds().contains(mouseposition.x, mouseposition.y) && Mouse::isButtonPressed(Mouse::Left))
    {
        p3section = 2;
    }

    if (profilebox.box.getGlobalBounds().contains(mouseposition.x, mouseposition.y) && Mouse::isButtonPressed(Mouse::Left))
    {
        profilepressed = true;
        page3 = false;
        page4 = true;
